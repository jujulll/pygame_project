...
def Start_screen():
    ...
    screen.blit(background, (272.5, 0))
...
def game(name):
    ...
    z = 0
    m = True
    start = Start_Finish(1, 15)
    finish = Start_Finish(3, 525)
    running = True
    flag = False
    y = 525
    part = 2
    while running:
        ...
        if m:
            if part == 2:
                pygame.draw.circle(screen, 'red', (675, y), 15)
            else:
                pygame.draw.circle(screen, 'red', (225, y), 15)
        s = 0
        if s == 3:
            s -= 3
        else:
